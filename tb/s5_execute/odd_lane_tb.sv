...
module odd_lane_tb;
  ...
  task automatic check_expect_quiet(input string name, input string detail);
    ...
    tb_field_in_u32("brch_pc",    brch_pc);
    tb_field_in_bit("mem_write",  mem_write);
    tb_field_in_u32("mem_addr",   mem_addr);
    tb_field_in_u32("mem_wdata",  mem_wdata);
    tb_field_in_u32("mem_besel",  {28'd0, mem_besel});
    tb_field_in_u32("link_pc",    link_pc);
    tb_field_in_u32("reg_wdata",  reg_wdata);
    tb_report_close(pass);
    if (pass) pass_cnt++; else fail_cnt++;
  endtask
