...
// instruction_cache_tb - dual fetch, LE insn assembly from byte cache.
module instruction_cache_tb;
  ...
  task automatic preload_word(input logic [31:0] byte_addr, input logic [31:0] word);
    logic [31:0] base;
    base = {byte_addr[31:2], 2'b00};
    dut.cache[base + 0] = word[7:0];
    dut.cache[base + 1] = word[15:8];
    dut.cache[base + 2] = word[23:16];
    dut.cache[base + 3] = word[31:24];
  endtask
  ...
  initial begin
    ...
    tb_banner("instruction_cache_tb - dual read, LE assembly");
