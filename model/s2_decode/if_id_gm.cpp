...
void IfIdGolden::apply_posedge(const Stim& s) {
  ...
  if (s.i0_fetch_valid) {
    ...
    out_.i0_instr_id = rv_gm::kInstrNop;
    out_.i0_pc_id = out_.i0_pc_target_id = 0;
    out_.i0_target_valid_id = out_.spec0_en_id = false;
  }
  if (s.i1_fetch_valid) {
    ...
    out_.i1_instr_id = rv_gm::kInstrNop;
    out_.i1_pc_id = out_.i1_pc_target_id = 0;
    out_.i1_target_valid_id = out_.spec1_en_id = false;
  }
}
...
void ifid_dpi_commit(void* h, int rst_n, int enable, int flush, int stall, int i0_fv,
  ...
  s.i0_fetch_valid = i0_fv; s.i1_fetch_valid = i1_fv;
